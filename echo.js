if (!!window.MutationObserver){//Checks if Mutation Observer is supported by Browser
	(new MutationObserver(M=>{//Sets an Observer on the HTML tag to watch for all new tags added and initialize an Observer on all tags with the ECHO attribute.
		let O = { // Variable is initialized within a function to prevent Global Collision, all further variables are keys of this object.
			CR:`color:red`,
			CB:`color:blue`,
			CO:`color:orange`,
			NULL:x=>(x !== null && x !== undefined), // Checks if argument is null or undefined
			GET:(E,x)=>(E.getAttribute(x)), // Gets an attribute
			SET:(E,x,y)=>(E.setAttribute(x,y)), // Sets an attribute
			ARR:x=>(Array.isArray(x)), // Checks if argument is an array
			NUM:x=>(typeof x !== `boolean` && Number.isFinite(Number(x))), // Checks if argument is a legal number
			TRIM:x=>(x.trim().replace(/\s\s+|\r|\n|\t|\f|\v|[,]/g,` `)),//Replaces all white space with a single space
			SPLIT:x=>(O.TRIM(x).split(` `)), // Splits string into an array and removes all excess white space
			...
			ECHO:(E,x)=>{
				...
				if (typeof O.CHK == `string` || O.NUM(O.CHK) || O.ARR(O.CHK)){//Checks if the ECHO attribute is passed an array by a varaible
					x = O.CHK;
					y = true;
				}
				...
				if (O.NUM(x)){x = Number(x)}
				if (typeof x == `string` && !O.ARR(x)){x = O.SPLIT(x)}//Converts a word string list to an array
			...
			WATCH:E=>{
				(new MutationObserver(R=>{
					...
					if (E.echo !== `[object Object]`){ // Check that HTML Echo Attribtue was not passed an Object directly
						if (R.attributeName === `echo` && !!E.oldArray){
							if (typeof E.oldArray !== `number`){ // Check that both requirements to store prechange values are present
								O.OBJ = {};
								E.oldArray.forEach(v=>{if (!!E.children[v]){O.OBJ[v] = E.children[v].outerHTML}}); // Store OldValue Data within an Object
							}
							else if (typeof E.oldArray === `number`){
								O.OBJ = [];
								for(O.INC = 0; O.INC < E.children.length; O.INC++){
									O.OBJ[O.INC] = E.children[O.INC].outerHTML
								}
							}
						}
						if (E.hasAttribute(`echo`)){
							...
							if (O.ARR(O.CHK)){
								O.CHK.forEach((v,i,a)=>{a[i] = O.TRIM(v).replace(/ /g,``)}) // Removes all white space within array values
								O.CHK = O.CHK.filter(x=>{return x.length > 0}); // Filters out all zero lenth array values
								}; // Removes all white space within the array
							...
							if (JSON.stringify(O.CHK) !== JSON.stringify(E.echoArray)){ // Checks for changes from the old Array and the new Array.
								...
								if (O.NUM(E.echoArray)){
									if (!O.OBJ){O.OBJ = []}
									for(O.INC = 0; O.INC < E.echoArray; O.INC++){
										E.insertAdjacentHTML( `beforeend` , (O.OBJ[O.INC] ? O.OBJ[O.INC] : E.codeHTML));
									}
								}
